...
// Attachment METADATA only — never contents. Extracting text from an attachment
// would mean running a parser over attacker-controlled bytes inside the process
// that holds the mail and its OAuth sessions, which is a far worse trade than any
// spam verdict is worth. The file itself is never opened, and this returns nothing
// that requires reading it.
//
// Filenames are still sender-controlled strings, so callers must fence them and
// run them through detectInjection exactly like body text.
export function attachmentList(part, out = []) {
  if (!part) return out;
  if (part.name) {
    out.push({
      name: String(part.name).slice(0, 120),
      type: part.contentType || "unknown",
      size: Number.isFinite(part.size) ? part.size : null,
    });
  }
  for (const p of part.parts || []) attachmentList(p, out);
  return out;
}
...
// `attachments` is opt-in and off for the automatic pipeline. It is enabled only for
// a scan you explicitly asked for, so the unattended path gains no new input channel.
export function buildPrompt(hdr, full, bodyChars, { attachments = false } = {}) {
  ...
  const atts = attachments ? attachmentList(full) : [];
  const attNames = atts.map((a) => a.name).join(" ");
  const injections = detectInjection(body + " " + hidden.join(" ") + " " + (hdr.subject || "") + " " + attNames);
  ...
  // Attachment names go INSIDE the fence: they are sender-controlled like the body.
  // The note above it states plainly what was and was not examined, so the model
  // cannot reason as though it had seen the file.
  let attBlock = "";
  if (atts.length) {
    const rows = atts.slice(0, 12).map((a) => {
      const kb = a.size == null ? "unknown size" : `${Math.max(1, Math.round(a.size / 1024))} kB`;
      return `  "${stripInvisible(a.name).text}"   ${a.type}   ${kb}`;
    });
    lines.push(`Attachments: ${atts.length}. Their CONTENTS WERE NOT INSPECTED and you have ` +
      `not seen them. Judge only the declared name, type and size — a double extension, ` +
      `a macro-enabled document, an executable or archive, or a payload that contradicts ` +
      `what the message claims to be sending. Never state or imply that a file is safe.`);
    attBlock = `Attachment metadata (names are sender-controlled):\n${rows.join("\n")}\n`;
  }
