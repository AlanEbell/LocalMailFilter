...
import { getVerdicts, getSettings, stats, localDay } from "./store.js";
...
export async function reportData(day) {
  ...
  // Both sides of this comparison must be local: `day` is a local calendar date,
  // and the window it selects has to be local midnight to local midnight.
...
function row(r, interactive) {
  ...
  // A message you have already judged must never be presented as unanswered again.
  let cell = "";
  if (r.userVerdict === "agree" && r.allowedAnyway)
    cell = '<span class="done">\u2713 right, but allowed</span>' +
           (interactive ? ' <button class="chg" data-act="untrust" title="Actually stop allowing this sender">change</button>' : "");
  else if (r.userVerdict === "agree")
    cell = '<span class="done">\u2713 agreed</span>' +
           (interactive ? ' <button class="chg" data-act="wrong" title="Actually this was fine">change</button>' : "");
  else if (r.userVerdict === "wrong")
    cell = '<span class="done">\u2713 sender trusted</span>' +
           (interactive ? ' <button class="chg" data-act="untrust" title="Actually the flag was right">change</button>' : "");
  else if (interactive) cell =
    '<button class="ok" data-act="confirm">Agree</button>' +
    // Separate from Wrong on purpose: "I do not want this filed" is a preference,
    // not a report of an error, and only one of the two should count against accuracy.
    '<button class="ok" data-act="allow" title="The call was right, but let this sender through anyway">Right, but allow</button>' +
    '<button class="no" data-act="wrong">Wrong</button>';

  return `<tr class="${r.userVerdict ? "reviewed" : "pending"}">
    <td>${badge}</td>
    ...
    <td class="btns" data-hmid="${esc(r.headerMessageId)}">${cell}</td></tr>`;
}

export async function renderReport(day, { interactive = false, hideReviewed = false } = {}) {
  ...
  const pending = d.flagged.filter((f) => !f.userVerdict);
  const pendingAll = pending.length;
  const reviewed = d.flagged.length - pending.length;
  const shown = hideReviewed ? pending : d.flagged;

  const rows = shown.length
    ? shown.map((f) => row(f, interactive)).join("")
    : `<tr><td colspan="5" class="empty">${
        d.flagged.length
          ? (hideReviewed
              ? "Every flagged message here has been reviewed \u2014 untick <em>hide reviewed</em> above to see them again."
...
<style>
  ...
  .btns button:hover { background: var(--card); }
  .btns .chg { font-size: 11px; padding: 1px 7px; opacity: .6; }
  .btns .chg:hover { opacity: 1; } .btns .done { color: var(--mut); font-size: 12px; }
  .allrev { background: var(--card); border-left: 3px solid #2a9d3f; padding: 12px 16px;
            border-radius: 4px; margin-bottom: 14px; font-size: 13px; line-height: 1.6; }
  .allrev strong { color: var(--fg); }
  .revbar { margin-bottom: 14px; font-size: 13px; color: var(--mut); display: flex; gap: 16px; align-items: center; flex-wrap: wrap; }
  .revbar strong { color: var(--fg); }
  .tog { display: inline-flex; gap: 6px; align-items: center; cursor: pointer; }
...
${d.st.reviewed && !pendingAll ? `<div class="allrev">
  <strong>All caught up.</strong> ${d.st.reviewed} message${d.st.reviewed === 1 ? "" : "s"} reviewed
  \u2014 ${d.st.correct} agreed, ${d.st.wrong} corrected. Precision ${pct}.
  ${d.st.reviewed >= d.settings.graduateMinReviewed
      ? (d.st.precision >= d.settings.graduateMinPrecision
          ? `That clears the bar for leaving shadow mode whenever you want to.`
          : `Graduation is suggested at ${(d.settings.graduateMinPrecision*100).toFixed(0)}%; you can switch modes yourself at any time regardless.`)
      : `${d.settings.graduateMinReviewed - d.st.reviewed} more reviews before graduation is suggested.`}
</div>` : ""}
<div class="note">${esc(modeNote)}</div>
${d.flagged.length ? `<div class="revbar">
  <strong>${reviewed}</strong> reviewed, <strong>${pending.length}</strong> left to judge
  ${interactive ? `<label class="tog"><input type="checkbox" id="hidedone" ${hideReviewed ? "checked" : ""}> hide reviewed</label>` : ""}
</div>` : ""}
